...
* Description: This plugin allows create Cost Calculator Contact Form 7 plugin.
* Version: 1.0
* Copyright: 2020
* Text Domain: cost-calculator-contact-form-7
...
if (!defined('CF7COSTCALOC_DOMAIN')) {
   	define('CF7COSTCALOC_DOMAIN', 'cost-calculator-contact-form-7');
}
if (!defined('DEBUG')) {
    define('DEBUG', 0);
}
	    ...
	    function CF7COSTCALOC_install_error() {
      	...
      	function CF7COSTCALOC_plugin_row_meta( $links, $file ) {
         	if ( CF7COSTCALOC_BASE_NAME === $file ) {
             	$row_meta = array(
                 	'rating'    =>  '<a href="https://www.xeeshop.com/calculate-contact-form-7/" target="_blank">Documentation</a> | <a href="https://www.xeeshop.com/support-us/?utm_source=aj_plugin&utm_medium=plugin_support&utm_campaign=aj_support&utm_content=aj_wordpress" target="_blank">Support</a> | <a href="https://wordpress.org/support/plugin/cost-calculator-contact-form-7/reviews/?filter=5" target="_blank"><img src="'.CF7COSTCALOC_PLUGIN_DIR.'/includes/images/star.png" class="costcf7oc_rating_div"></a>',
             	);
...
add_action( 'plugins_loaded', 'CF7COSTCALOC_load_textdomaindd' );
function CF7COSTCALOC_load_textdomaindd() {
    load_plugin_textdomain( 'cost-calculator-contact-form-7', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' ); 
}

function CF7COSTCALOC_load_my_own_textdomaindd( $mofile, $domain ) {
    if ( 'cost-calculator-contact-form-7' === $domain && false !== strpos( $mofile, WP_LANG_DIR . '/plugins/' ) ) {
        $locale = apply_filters( 'plugin_locale', determine_locale(), $domain );
        $mofile = WP_PLUGIN_DIR . '/' . dirname( plugin_basename( __FILE__ ) ) . '/languages/' . $domain . '-' . $locale . '.mo';
    }
    return $mofile;
}
add_filter( 'load_textdomain_mofile', 'CF7COSTCALOC_load_my_own_textdomaindd', 10, 2 );